...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ lang }) => {
    ...
    // Initial message handled via Effect or function to respond to Lang change, 
    // but simpler to just reset if lang changes or keep history. 
    // For now, we will just translate the initial state.
    ...
    // Reset/Set initial message when language changes
    useEffect(() => {
        setMessages([
    ...
    // Scroll Logic to avoid Footer overlap
    useEffect(() => {
        const handleScroll = () => {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            // System context updated to be language aware or general
    ...
    // Position logic: 
    // AR (RTL) -> Left side (standard for this design)
    // EN (LTR) -> Right side (flipped to match standard LTR chat position)
    // UPDATED: Reverted to edge position (left-4) as requested "far left"
    const positionClasses = lang === 'ar' 
        ? 'left-4 md:left-6 items-end origin-bottom-left' 
        : 'right-4 md:right-6 items-end origin-bottom-right';
    ...
    return (
        ...
        >
            ...
            {/* 1. Chat Interface (UP Mode) */}
            {isOpen && placement === 'up' && chatWindowContent}

            {/* 2. Trigger Button / Avatar */}
            <div 
                onClick={() => setIsOpen(!isOpen)}
                className="cursor-pointer group flex items-center gap-3 relative z-50"
            >
                {!isOpen && (
                    <div className="hidden md:flex items-end gap-2 animate-bounce mb-8 -ml-12" style={{ animationDuration: '3s' }}>
                        {/* Text pill - Always first in DOM, but flex direction handles visual order */}
                        <div className="bg-sanad-soft text-white px-6 py-2 rounded-full font-bold text-sm shadow-sm whitespace-nowrap mb-2">
                            {t.triggerText}
                        </div>
                        {/* Dots - Always second in DOM */}
                ...
                {/* Trigger Button Container (The Avatar - Floating Style) */}
                <div className={`w-32 h-32 md:w-52 md:h-52 relative transform transition-transform duration-300 z-50 ${isOpen ? 'scale-90 rotate-6' : 'group-hover:scale-110'}`}>
                    {/* SanadCharacter with Drop Shadow to pop against background since we removed the circle */}
                    <SanadCharacter className="w-full h-full object-contain drop-shadow-2xl filter" />
                </div>
            </div>

            {/* 3. Chat Interface (DOWN Mode) */}
